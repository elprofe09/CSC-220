...
public class Library <Type> {
   ...
   */
  public Type lookup(long isbn) {//changed from String
   ...
   *  The only missing part is for you to complete the implementation of compare method inside IsbnComparator that implements comparator interface. 
   */
  public ArrayList < LibraryBook < Type >> getInventoryList() {
	  ...
	  * You need to think about how to implement this method and what this method needs to return
	  */
	 protected class IsbnComparator implements Comparator < LibraryBook < Type >> {

	     // FILL IN - write the compare method
		 public int compare(LibraryBook < Type > libraryCopy,LibraryBook < Type > library) {
			 //return (int)(libraryCopy.getIsbn()-library.getIsbn());
			 if(libraryCopy.getIsbn()==library.getIsbn())
				 return 0;
			 else if(libraryCopy.getIsbn()<library.getIsbn())
				 return -1;
	 ...
	 public ArrayList<LibraryBook<Type>> getOrderedByAuthor() {
	   // FILL IN -- do not return null
		 ArrayList < LibraryBook < Type >> libraryCopyByAuthor = new ArrayList < LibraryBook < Type >> ();
	 ...
	 public ArrayList < LibraryBook < Type >> getOverdueList(int month, int day,int year) {
			    ...
			    ArrayList < LibraryBook < Type >> overDueLibraryBooks = new ArrayList < LibraryBook < Type >> ();
			    DueDateComparator Comparator = new DueDateComparator();
			    // FILL IN -- do not return null
			    GregorianCalendar date = new GregorianCalendar(month, day, year);
			    for (int i = 0; i < library.size(); i++) {
			        if (library.get(i).getDueDate() != null) { 
			            int comparingDate = date.compareTo(library.get(i).getDueDate());
			            if (comparingDate < 0)
			                overDueLibraryBooks.add(library.get(i));//read if there is more to do with the compare method 

			        }
			    }

			    sort(overDueLibraryBooks, Comparator); //collection.sort
			    return overDueLibraryBooks;
			}
	  ...
	  */
	  ...
	  protected class AuthorComparator implements Comparator<LibraryBook<Type>> {
		  public int compare(LibraryBook < Type > libraryCopy,LibraryBook < Type > library) {
			  String author1 = libraryCopy.getAuthor();
			  String author2 = library.getAuthor();
			  int result = author1.compareTo(author2);
			  if(result == 0) {
				  String bookOfAuthor1 = libraryCopy.getTitle();
				  String bookOfAuthor2 = library.getTitle();
				  int tieBreaker = (bookOfAuthor1.compareTo(bookOfAuthor2));
				  if(tieBreaker>0) {
					  return 1;
				  }
					  ...
					  System.out.print("books are the same");
					  return 0;
				  }
				  ...
				  return 1;
			  //if(libraryCopy.getAuthor()==library.getAuthor()) {
				///  if(libraryCopy.getTitle()==library.getTitle())
			 // }
		  }
	    // FILL IN - write the compare method
	  }
	  ...
	  */
	  ...
	  protected class DueDateComparator implements Comparator < LibraryBook < Type >> {
		     public int compare(LibraryBook < Type > libraryCopy, LibraryBook < Type > library) {
		         GregorianCalendar BookDate = libraryCopy.getDueDate();
		         GregorianCalendar BookDate2 = library.getDueDate();
		         int resultOfDateCompare = BookDate.compareTo(BookDate2);
		         if (resultOfDateCompare < 0) {
		             return -1;
		         } else if (resultOfDateCompare > 0) {
		             return 1;
		         } else
		             return 0;
		     }
